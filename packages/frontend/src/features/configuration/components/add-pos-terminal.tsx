...
import { useCreatePos } from '@/lib/api/queries/pos-terminal/create-pos'
...
export interface AddPosTerminalProps {
  onCompleted?: () => void
...
const AddPosTerminal = ({ onCompleted, onError }: AddPosTerminalProps) => {
  const { close } = useDialogContext()

  const { mutate: createPos, isPending } = useCreatePos({
    onSuccess() {
      toastSuccess('Tạo POS thành công')
      close()
      onCompleted?.()
    },
    onError(error) {
      const message = error.response?.data.message || 'Tạo POS thất bại'
      toastError(message)
      onError?.(error)
    },
  })
