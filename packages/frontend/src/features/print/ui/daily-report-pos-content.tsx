...
const DataReportPOSContent = ({
  ...
  return (
    <PrintLayout className="daily-report-pos" ref={printRef}>
      <div className="flex flex-col text-xs">
        <h2 className="!text-xl !font-langfarm text-left">BÁO CÁO NỘP TIỀN POS</h2>
        <span> Thu Ngân: {reportData?.cashierName}</span>
        <span> Người in: {`${currentUser?.data?.lastName} ${currentUser?.data?.firstName}`}</span>
        <span> Thời gian in: {appDayJs().format('DD/MM/YYYY HH:mm:ss')}</span>
      </div>
...
export default memo(DataReportPOSContent)
