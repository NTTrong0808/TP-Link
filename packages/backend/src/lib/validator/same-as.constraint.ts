...
export class SameAsConstraint implements ValidatorConstraintInterface {
  validate(value: any, validationArguments?: ValidationArguments): boolean {
    const targetValues = (validationArguments?.object || {}) as Record<
      string,
      any
    >;
    const constraints = validationArguments
      ?.constraints?.[0] as SameAsConstraintOptions;
  ...
  defaultMessage?(validationArguments?: ValidationArguments): string {
    const constraints = validationArguments
      ?.constraints?.[0] as unknown as SameAsConstraintOptions;
...
export function SameAs(
  options: SameAsConstraintOptions,
